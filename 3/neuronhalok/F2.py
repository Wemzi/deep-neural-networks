...
def create_cl_model_2(hp):
  ...
  cl_model.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=hp_learning_rate
...
def create_cl_model():
  ...
  cl_model.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=0.006
                                                     ), loss="categorical_crossentropy", metrics=[prec,'accuracy',rec],)
...
earlystopping_callback = tf.keras.callbacks.EarlyStopping(monitor="val_accuracy", patience=300)
history = cl_model.fit(x_train,y_onehot_train,validation_data=(x_val,y_onehot_val),batch_size=128,epochs=9000,callbacks=[earlystopping_callback])
...
tuner = kt.Hyperband(create_cl_model_2,
